...
- (void)startScreenRecordingWithScreenView:(UIView *)screenView failureBlock:(FailureBlock)failureBlock
{
    ...
    _filePath = [NSString stringWithFormat:@"%@/%d.mp4",kMoviePath,[NSDate now].timeIntervalSince1970];
    // 04 视频转换设置
    NSError *error = nil;
    _videoWriter = [[AVAssetWriter alloc] initWithURL:[NSURL fileURLWithPath:_filePath]
...
//    [self performSelector:@selector(threadend) onThread:_timer_thread withObject:nil waitUntilDone:YES];
    [_timer invalidate];
    _timer = nil;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), _serial_writeVideo_queue, ^{
        [_writerInput markAsFinished];
        [_videoWriter finishWritingWithCompletionHandler:^{
            NSLog(@"Successfully closed video writer");
            NSLog(@"videowriter status: %d", _videoWriter.status);
            if (_videoWriter.status == AVAssetWriterStatusCompleted) {
                NSLog(@"成功");
                if (self.finishBlock != nil) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        self.finishBlock(nil, _filePath);
                    });
                }
            } else {
                NSLog(@"失败");
